        ...
        public void ShowCurrenteDockStatus()
        {
            ...
            for (int i = 0; i < dockedBoats.Length; i++)
            {
                ...
                else
                {
                    ...
                    if(dockedBoats[i] is RowBoat)
                    {
                        var rowBoat = dockedBoats[i] as RowBoat;
                        if(rowBoat.Neighbour != null)
                        {
                            Console.WriteLine($"Slot{i}: {rowBoat.Neighbour.BoatId}, Days left: {rowBoat.Neighbour.DaysToStay}");
                        }
                    }
                }
            }
